...
- **Gas optimization:** shared Gaussian calculations and hoisted reserve checks retain checked arithmetic and per-evaluation domain checks. Exact differential tests cover 1,246 inputs, not the entire input space. The router is immutable; migration changes its address and Aqua authorization, not the market. Other makers' old authorizations remain theirs to manage. See [migration details](gas-optimization.md).
