...
The browser workspace is keyed by chain, market, router and the manifest's seed-order hash. It stores created strategy definitions, the selected strategy and receipt entries in localStorage. Strategy restoration rebuilds the expected order and checks its hash against the fixed deployment; saved data cannot replace settlement/token/RPC addresses. The seed remains selectable under Liquidity. Fresh deployments get independent workspaces. Legacy caches restore receipts only because they did not identify the router; custom liquidity authorizations remain on their original router. See [gas optimization](gas-optimization.md) for the seed migration and recovery instructions.
